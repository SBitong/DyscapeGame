...
class Game:
    def __init__(self):
        ...
        self.fourthLevel = TheBrokenBridge(self.screen, self.gameStateManager, game_id=4)
        self.fifthLevel = TheRhymeanGarden(self.screen, self.gameStateManager, game_id=5)
        self.sixthLevel = ForestOfNolite(self.screen, self.gameStateManager, game_id=6)
        self.seventhLevel = EchoingChambers(self.screen, self.gameStateManager, game_id=7)
        self.eighthLevel = EighthLevel(self.screen, self.gameStateManager, game_id=8)
        self.ninthLevel = NinthLevel(self.screen, self.gameStateManager, game_id=9)
        self.finalLevel = FinalLevel(self.screen, self.gameStateManager, game_id=10)
        self.ending = Ending(self.screen, self.gameStateManager, game_id=11)
        self.states = {
                ...
                if event.key == pygame.K_RETURN:
                    ...
                    if user_text:  # If the user entered a name
                        ...
                        if game_id:
                            ...
                            # Go directly to The Unknown Toad level
                            gameStateManager.set_state(TheUnknownToad(display, gameStateManager, game_id))
...
class GameStart:
    ...
    def start_new_game(self, game_name):
        ...
        if game_id is not None:
            print(f"Starting new game with Game ID {game_id} at The Unknown Toad.")
            # Pass game_id as the identifier here
            self.gameStateManager.set_state(TheUnknownToad(self.display, self.gameStateManager, game_id))
...
class Database:
    ...
    def create_and_start_new_game(self, name, level="The Unknown Toad"):
        with self.conn:
            cursor = self.conn.cursor()
            ...
            cursor.execute('INSERT INTO games (name, creation_date, last_played_date, current_level) VALUES (?, ?, ?, ?)',
                           (name, creation_date, creation_date, level))
            self.conn.commit()
            print("New game created with ID:", cursor.lastrowid)
    ...
    def get_max_unlocked_level(self, game_id):
        """Fetch the highest unlocked level for the specified game ID."""
        with sqlite3.connect('game_data.db') as conn:
            cursor = conn.cursor()
            cursor.execute('SELECT current_level FROM games WHERE id = ?', (game_id,))
            result = cursor.fetchone()
            if result:
                fetched_level = result[0].strip().lower()  # Normalize fetched level
                # Map level names to numerical values
                level_mapping = {
                ...
                return level_mapping.get(fetched_level, 1)  # Default to level 1 if not found
            return 1  # Default to level 1 if no result found

    def continue_game(self, game_id):
        """
        Continue a saved game by loading the maximum unlocked level.
        """
        ...
        if game_data:
            current_level_str = game_data[4].strip().lower()  # Normalize the current level string
            print(f"Fetched current level string: {current_level_str}")  # Debugging statement

            # Use the global level mapping to determine max unlocked level
            level_mapping = {
                'the unknown toad': 1, 'lava rush': 2, 'sylle lagoon': 3, 'the broken bridge': 4,
                'the rhymean garden': 5, 'forest of nolite': 6, 'echoing chambers': 7, 'eighth level': 8,
                'ninth-level': 9, 'final-level': 10, 'ending': 11
            ...
            # Get the maximum unlocked level using the mapping
            max_unlocked_level = level_mapping.get(current_level_str, 1)
            print(f"Determined Max Unlocked Level: {max_unlocked_level}")  # Debugging statement

            # Restrict to 8 levels for display but retain progress for continuation beyond level 8
            display_max_level = min(max_unlocked_level, 8)
            print(f"Displaying levels up to: {display_max_level}")  # Debugging statement

            # Initialize the level selection page with the determined max level
...
class LevelSelectionPage:
    def __init__(self, display, gameStateManager, game_id, max_unlocked_level):
        ...
        self.back_button_rect = self.back_button.get_rect(topleft=(50, 5))
    ...
    def get_max_unlocked_level(self, game_id):
        ...
        with sqlite3.connect('game_data.db') as conn:
            ...
            if result:
                fetched_level = result[0].strip().lower()  # Normalize fetched level
                # Level mapping for progression
                level_mapping = {
                    "the unknown toad": 1, "lava rush": 2, "sylle lagoon": 3, "the broken bridge": 4,
                    "the rhymean garden": 5, "forest of nolite": 6, "echoing chambers": 7, "eighth level": 8,
                    "ninth-level": 9, "final-level": 10, "ending": 11
                }
                # Convert fetched level to its corresponding numerical value
                max_unlocked_level = level_mapping.get(fetched_level, 1)
                return max_unlocked_level  # Return the actual highest level unlocked
            return 1  # Default to level 1 if no result found

    def display_page(self):
        """Display the level selection page."""
        self.display.blit(self.background_image, (0, 0))
        self.display.blit(self.back_button, self.back_button_rect)

        # Fetch highest unlocked level
        self.max_unlocked_level = self.get_max_unlocked_level(self.game_id)

        for i in range(1, 9):  # Levels 1–8
    ...
    def handle_events(self):
        ...
        level_classes = {
            1: TheUnknownToad, 2: LavaRush, 3: SylleLagoon, 4: TheBrokenBridge,
...
class TheUnknownToad:
    def __init__(self, display, gameStateManager, game_id):
        ...
        hover_sound_path = os.path.join('audio', 'mouse_hover_effect_01.mp3')  # Adjust the path as needed
        self.hover_sound = pygame.mixer.Sound(hover_sound_path)
    ...
    def show_end_screen(self):
        ...
        while running:
            ...
            # Draw buttons
            if self.win:
                self.update_progress_in_database("Lava Rush")
                    ...
                    if self.win:
                        if next_level_button.collidepoint(event.pos):
                            ...
                            self.gameStateManager.set_state(
                                LavaRush(self.display, self.gameStateManager, self.game_id))
                            running = False
    ...
    def run(self):
        ...
        while running:
            ...
            question_number_text = f"Question: {current_question_number}/{total_questions}"
            question_number_surface = self.font.render(question_number_text, True, self.white)
            self.display.blit(question_number_surface, (1050, 20))  # Position at the top left
            ...
            if not any(rect.collidepoint(mouse_pos) for rect in self.choice_rects):
...
class LavaRush:
    def __init__(self, display, gameStateManager, game_id):
        ...
        # Define colors
        self.normal_color = (254, 223, 0)  # Yellow
        self.hover_color = (200, 200, 0)  # Darker yellow
    ...
    def show_end_screen(self):
        ...
        if self.win:
            self.update_progress_in_database("Sylle Lagoon")
                ...
                for event in pygame.event.get():
                    if event.type == pygame.QUIT:
                        ...
                        if next_level_button.collidepoint(event.pos):
                            # Go to the next level
                            self.gameStateManager.set_state(
                                SylleLagoon(self.display, self.gameStateManager, self.game_id))
                            running = False
    ...
    def run(self):
        ...
        while running:
            ...
            # Button color logic
            submit_button_color = self.normal_color
            reset_button_color = self.normal_color

            # Check for mouse hover on buttons
            mouse_pos = pygame.mouse.get_pos()
            if self.submit_button_rect.collidepoint(mouse_pos):
                submit_button_color = self.hover_color
            if self.reset_button_rect.collidepoint(mouse_pos):
                reset_button_color = self.hover_color
...
class SylleLagoon:
    ...
    def run_dialogue_strip_2(self):
        ...
        dialogue_data = [
            {"name": "Lexi the Owl",
             "text": "We cant just swim to this lagoon because piranhas and other sea monsters\n will eat us alive.",
             "audio": "owl-talking-24.mp3"},
            {"name": "Lexi the Owl",
             "text": "And it seems like we can't go around the lagoon either since we will get\n caught up by the burning forest.",
             "audio": "owl-talking-25.mp3"},
            {"name": "You",
             "text": "I think I got it, Lexi! The only way we can get across this water is to step through this footholes.",
             "image": char1_image},
            {"name": "You",
             "text": "Every foothole has a time interval before they open, and some dont even open at all. We need to take advantage of that.",
             "image": char1_image},
            {"name": "Lexi the Owl", "text": "Wow, adventurer! You really are observant!",
             "audio": "owl-talking-26.mp3"},
            {"name": "Lexi the Owl", "text": "So we just need to step onto the foothole that doesnt open at all?",
             "audio": "owl-talking-27.mp3"},
    ...
    def show_end_screen(self):
        ...
        message_rect = message_surface.get_rect(center=(self.display.get_width() // 2, self.display.get_height() // 3  - 100))
        ...
        if self.win:
            self.update_progress_in_database("The Broken Bridge")
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    # Check if Next Level button is clicked (if player won)
                        ...
                        # Go to the next level
                        self.gameStateManager.set_state(
                            TheBrokenBridge(self.display, self.gameStateManager, self.game_id))
                        waiting = False
    ...
    def run(self):
        ...
        while running:
            ...
            counter_surface = pygame.font.Font('fonts/ARIALBD.TTF', 40).render(question_counter_text, True,
                                                                               (255, 255, 255))
...
class TheRhymeanGarden:
    ...
    def run_dialogue_strip_2(self):
        ...
        dialogue_data = [
            {"name": "Lexi the Owl", "text": "This will be the perfect time that we will use your sword, adventurer.",
             "audio": "owl-talking-33.mp3"},
            {"name": "Lexi the Owl",
             "text": "Not only physical things, but also animals nearby are being affected by the curse.",
             "audio": "owl-talking-34.mp3"},
            {"name": "You", "text": "What can we do, Lexi? This is my first time holding a sword.",
             "image": char1_image},
            ...
            {"name": "Lexi the Owl", "text": "Maybe there is some way we can defeat them effectively.",
             "audio": "owl-talking-36.mp3"},
            {"name": "You", "text": "Maybe a perfect combo will do a thing? like RHYMING words?", "image": char1_image},
            {"name": "Lexi the Owl", "text": "That's a good idea! This garden is called Rhymean Garden, after all.",
             "audio": "owl-talking-37.mp3"},
    ...
    def show_how_to_play(self):
        ...
        # Load the how-to-play image and scale it
        how_to_play_image = pygame.image.load(
            os.path.join('graphics', 'how-to-play(rhymean-garden).png')).convert_alpha()
        how_to_play_image = pygame.transform.scale(how_to_play_image, (1280, 720))  # Scale to 1280x720
    ...
    def show_end_screen(self):
        ...
        if self.rounds_completed >= self.max_rounds:
            self.update_progress_in_database("Forest of Nolite")
                ...
                if event.type == pygame.QUIT:
                    ...
                    if event.type == pygame.MOUSEBUTTONDOWN:
                        if self.restart_button.collidepoint(event.pos):
                            ...
                            # Go to the next level
                            self.gameStateManager.set_state(
                                ForestOfNolite(self.display, self.gameStateManager, self.game_id))
                            running = False
...
class ForestOfNolite:
    ...
    def run_dialogue_strip_2(self):
        ...
        dialogue_data = [
            ...
            {"name": "Lexi the Owl", "text": "This is a vibrant forest, traveler. Confusion drove away the fireflies",
             "audio": "owl-talking-39.mp3"},
            {"name": "Lexi the Owl", "text": "As a result, the bright forest became a dull one.",
             "audio": "owl-talking-40.mp3"},
            {"name": "Lexi the Owl", "text": "We have to save this forest, and its fireflies.",
             "audio": "owl-talking-41.mp3"},
    ...
    def show_end_screen(self):
        ...
        running = True
        while running:
            self.display.fill((0, 0, 0))  # Fill black bg background

            # Determine the message and button configurations
            if self.win:
                message = "You win!"
                self.update_progress_in_database("Echoing Chambers")
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    if self.win and next_level_button.collidepoint(event.pos):
                        ...
                        # Go to the next level
                        self.gameStateManager.set_state(
                            EchoingChambers(self.display, self.gameStateManager, self.game_id))
...
class EchoingChambers:
    ...
    def run_dialogue_strip_2(self):
        ...
        dialogue_data = [
            {"name": "Lexi the Owl",
             "text": "This is the echoing cave. And right after this cave is the center of Dyscape, where the tower is.",
             "audio": "owl-talking-43.mp3"},
                ...
                for event in pygame.event.get():
                    if event.type == pygame.QUIT:
                        ...
                        if self.next_level_button.collidepoint(mouse_pos):
                            # Go to the next level
                            self.gameStateManager.set_state(
                                EighthLevel(self.display, self.gameStateManager, self.game_id))
                            running = False
...
class EighthLevel:
    ...
    def check_level_completion(self):
        if self.win:  # This should check if the player has won the eighth level
            ...
            self.transition_to_next_level("ninth-level")
        else:
    ...
    def show_end_screen(self):
        ...
        if self.win:
            self.update_progress_in_database("Final Level")
            # If player wins, add a Next Level button
            self.next_level_button = pygame.Rect(self.display.get_width() // 2 - (250 // 2),
                                                 self.display.get_height() // 2 - 70, 250, 50)
    ...
    def transition_to_next_level(self, current_level):
        """Directly transition to the ninth level."""
        next_level_class = NinthLevel  # Replace this with the actual class name for the ninth level

        print(f"Transitioning from {current_level} to ninth-level.")

        try:
            # Access game_id from the current level instance
            game_id = self.game_id  # Ensure self.game_id exists in the current level class

            # Update the database to reflect progress
            self.database.update_progress_in_database(game_id, "ninth-level")

            # Directly instantiate the next level and pass it to set_state
            self.gameStateManager.set_state(next_level_class(self.display, self.gameStateManager, game_id))
            print("Successfully transitioned to the ninth level.")
        except Exception as e:
            print(f"Error during transition to the ninth level: {e}")
    ...
    def run(self):
        ...
        while running:
            ...
            if self.game_over:  # If the player lost
                ...
                pygame.mixer.music.stop()
                self.transition_to_next_level('eighth-level')  # Directly transition to the ninth level
                running = False
            else:
    ...
    def run(self):
        ...
        while running:
            ...
            if self.win:
                ...
                running = False
                print("Ninth level completed. Transitioning to the final level.")
            self.update_animation()
